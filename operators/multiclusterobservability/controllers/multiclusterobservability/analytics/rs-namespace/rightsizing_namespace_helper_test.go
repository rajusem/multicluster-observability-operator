...
func TestCleanupRSNamespaceResources_WithBindingUpdated(t *testing.T) {
	...
	k8sClient := fake.NewClientBuilder().WithScheme(scheme).WithObjects(
		&policyv1.PlacementBinding{ObjectMeta: metav1.ObjectMeta{Name: rsnamespace.RSPlacementBindingName, Namespace: mockRsNamespace}},
		&clusterv1beta1.Placement{ObjectMeta: metav1.ObjectMeta{Name: rsnamespace.RSPlacementName, Namespace: mockRsNamespace}},
		&policyv1.Policy{ObjectMeta: metav1.ObjectMeta{Name: rsnamespace.RSPrometheusRulePolicyName, Namespace: mockRsNamespace}},
	).Build()

	rsnamespace.CleanupRSNamespaceResources(context.TODO(), k8sClient, mockRsNamespace, true)
...
func TestCleanupRSNamespaceResources_WithBindingNotUpdated(t *testing.T) {
	...
	k8sClient := fake.NewClientBuilder().WithScheme(scheme).WithObjects(
		...
		&clusterv1beta1.Placement{ObjectMeta: metav1.ObjectMeta{Name: rsnamespace.RSPlacementName, Namespace: mockRsNamespace}},
		&policyv1.Policy{ObjectMeta: metav1.ObjectMeta{Name: rsnamespace.RSPrometheusRulePolicyName, Namespace: mockRsNamespace}},
		&corev1.ConfigMap{ObjectMeta: metav1.ObjectMeta{Name: rsnamespace.RSConfigMapName, Namespace: configMapNS}},
	).Build()

	rsnamespace.CleanupRSNamespaceResources(context.TODO(), k8sClient, mockRsNamespace, false)
